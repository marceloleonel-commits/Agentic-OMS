...
| **Working title/Commercial Name:** Agentic Workflow — Gerenciar Experiências (previously referred to as Gerenciar Order Jobs / Controle de Fluxos / Order Workflow / Dynamic Workflow) |
...
**Experiência** — A named operational model for a specific fulfillment scenario (e.g., Entrega em domicílio, Retirada na loja, Entrega digital). Experiências are defined in the "Gerenciar Experiências" canvas; at runtime, the system automatically creates an instance for each combination of fulfillment model + SLA + seller within an order, routing the matching items through that flow. Merchants configure base templates; they do not build workflows from scratch.
...
**1. Experiências with fixed Marcos.** The primary configuration unit is the **Experiência** — a named operational model designed for a specific fulfillment scenario (e.g., Entrega em domicílio, Retirada na loja, Entrega digital). An Experiência is created automatically for each combination of fulfillment parameters within an order: items sharing the same fulfillment model, SLA, and seller follow the same flow. Each Experiência contains exactly 4 **marcos** (milestones that become kanban columns): Confirmação de Pagamento, Preparando Itens, NFes Emitidas, and Recebido pelo Cliente. Merchants select a base template and customize — they do not build workflows from scratch. VTEX ships 3 pre-built Experiências plus Troca e Devolução: Entrega em domicílio 🚚, Retirada na loja 🏪, and Entrega digital 💻.
...
**5. Gerenciar Experiências (visual workflow canvas).** A visual Kanban board accessed via the "Gerenciar Experiências" nav item, where each column is a marco and cards within columns are tasks. Operators can create a new Experiência (choosing a base template or empty structure), add tasks within a marco, rename marcos, split or delete tasks — all via conversational side-panel chat. The "+ Nova Experiência" creation flow guides merchants through 6 steps: name → base template → icon → description → marcos preview → confirm. A context-aware supplier catalog auto-suggests responsible parties based on task name.
...
| Jun 2026 | Aligned to prototype: renamed "Order Job" → "Experiência" and "Gerenciar Order Jobs" → "Gerenciar Experiências" throughout; added Experiência definition to Vision Concepts; reduced native Experiências from 4 to 3 (removed Entrega pela loja / ship-from-store); added system-defined grouping criteria (fulfillment model + SLA + seller) to Key Capability #1; updated Key Capability #5 title and creation flow label |