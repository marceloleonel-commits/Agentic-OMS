...
function computeGroupMilestones(groupItems, order) {
  ...
  if (hasReturn) {
    ...
    milestones.push({ label:'Troca e Devolução', icon:'↩️', status: statusOf(retTasks) || 'em_andamento' });
  }
  return milestones;
}
...
function openConnectorPanel(wfId) {
  ...
  const slotsHtml = SLOTS.map(slot => {
    ...
    const stateLabel = !configured ? 'Não configurado' : b.state === 'active' ? 'Ativo' : 'Erro';
    ...
    const testedHtml = b?.lastTested
      ? `<div style="font-size:10.5px;color:#888;margin-top:8px">Último teste: ${b.lastTested} <span style="color:${b.lastTestStatus==='success'?'#15803d':'#dc2626'}">● ${b.lastTestStatus==='success'?'OK':'Falhou'}</span></div>`
      : '';
      ...
      <div style="border:1px solid ${configured?'#e5e7eb':'#f0f0f0'};border-radius:10px;padding:14px;margin-bottom:12px;background:${configured?'#fff':'#fafafa'}">
        ...
        <div style="display:flex;align-items:center;gap:8px">
          ...
          <div style="flex:1;min-width:0">
            ...
            <div style="font-size:13.5px;font-weight:700;color:${configured?'#1a1a1a':'#9ca3af'}">${configured?connector.displayName:'Não configurado'}</div>
          </div>
  ...
  document.getElementById('sp-title').textContent = 'Conectores & Tarefas';
  document.getElementById('sp-body').innerHTML = `
  ...
  document.getElementById('sp-footer').innerHTML = `
    <button class="btn btn-secondary" style="flex:1;justify-content:center" onclick="closeSidePanel()">Fechar</button>`;
  openSidePanel();
}
...
function testConnector(wfId, slot) {
  ...
  if (connector.id === 'conn-intelipost') {
    mockData = getMockIntelipostResponse();
  } else {
    ...
    mockData = fn ? fn() : { request:{ method:'GET', url:'https://api.example.com/test' }, response:{ status:200, body:{ ok:true } } };
  }

  const statusColor = mockData.response.status < 300 ? '#15803d' : '#dc2626';
  const statusBg    = mockData.response.status < 300 ? '#f0fdf4' : '#fef2f2';
  ...
  const reqBodyHtml = mockData.request.body
    ? `<div style="font-size:11px;font-weight:600;color:#555;margin:6px 0 3px">Body</div><pre style="font-size:10.5px;background:#f9fafb;border:1px solid #e5e7eb;border-radius:6px;padding:8px;overflow-x:auto;line-height:1.5">${prettyJson(mockData.request.body)}</pre>`
    : '';
  const headersHtml = Object.entries(mockData.request.headers||{}).map(([k,v]) =>
  ...
  document.getElementById('ct-body').innerHTML = `
    <div style="margin-bottom:14px">
      ...
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px">
        <span style="font-size:11px;font-weight:700;padding:2px 7px;border-radius:4px;background:#0c6fcd14;color:#0c6fcd">${mockData.request.method}</span>
        <code style="font-size:10.5px;color:#1a1a1a;word-break:break-all">${mockData.request.url}</code>
      </div>
    ...
    <div>
      ...
      <div style="display:inline-flex;align-items:center;gap:5px;margin-bottom:8px;padding:3px 10px;border-radius:10px;background:${statusBg};border:1px solid ${statusColor}40;color:${statusColor};font-size:11px;font-weight:700">
        ● HTTP ${mockData.response.status}
      </div>
      <pre style="font-size:10.5px;background:#f9fafb;border:1px solid #e5e7eb;border-radius:6px;padding:8px;overflow-x:auto;line-height:1.5;max-height:260px">${prettyJson(mockData.response.body)}</pre>
    </div>`;
  ...
  document.getElementById('ct-footer').innerHTML = `
    <button class="btn btn-secondary" onclick="closeModal('modal-connector-test')">Fechar</button>
  ...
  el.innerHTML = configured.map(slot => {
    ...
    return `
      <div style="display:flex;align-items:center;gap:10px;padding:10px 12px;background:#f9fafb;border:1px solid #e5e7eb;border-radius:8px">
        ...
        <span style="font-size:10.5px;font-weight:600;padding:2px 8px;border-radius:10px;background:#f0fdf4;color:#15803d;border:1px solid #bbf7d0;white-space:nowrap">● Ativo</span>
      </div>`;
  }).join('');
}
