...
- **Acceptance Criteria**:
  ...
  - **Given** a lot row, **when** I open its overflow menu, **then** it lists **Editar**,
    **Marcar como recebido**, **Histórico de atualizações** and, after a separator,
    **Excluir lote** as critical — and picking any of them only closes the menu.
...
| Edge — inert affordance | Listing rendered | Click "Criar lote futuro", then "Por SKU", then an item of a row menu | Nothing happens in all three cases; no navigation, no console error |
...
- **FR-3**: Lot rows expose: lot code and name, destination (seller and warehouse),
  total quantity, total reservations, arrival date, status tag and an overflow menu.
...
- **FR-4a**: **Reservas** reports units already sold in orders and held against the lot.
  ...
  in the header saying so, and is right-aligned as a numeric column. Reservations are
  independent of status: a cancelled lot still carries what had been sold before it was
  cancelled.
...
- **FR-14a**: The **row overflow menu** is a Shoreline `Menu` anchored to the row's
  ...
  atualizações** and, after a `MenuSeparator`, **Excluir lote** with `data-critical`.
  **Marcar como recebido** is omitted for lots whose status is `received` or `cancelled`:
  the lot's past is not marked again. Every item is inert — none of the four has a screen
  in this scope, so picking one only closes the menu and returns focus to the trigger.
