...
- **FR-3**: Lot rows expose: lot code and name, destination (seller and warehouse),
  ...
  - **Chegada** holds the expected arrival while the lot is scheduled and **the date it was
    received** once it has been (`002.5` FR-17, INV-8). The column keeps one meaning per
    row — when the lot arrived, expected or actual — instead of preserving a forecast that
    the receipt already settled.
...
- **FR-4a**: **Reservas** reports units already sold in orders and held against the lot.
  - **The column is a mirror, and this module is not its owner.** Reservations are made in
    **physical inventory**; future inventory reads them and shows them next to the lot so
    the operator can judge coverage without opening the inventory screen and doing the
    subtraction in their head. **No flow here changes an existing reservation** — not this
    listing, not the receipt (`002.5` FR-18). The creation form is the one place that
    writes the field at all, and only to be born at **zero**: a lot that did not exist
    until now has had nothing sold against it (`002.3` INV-6). From then on the number
    moves only when physical inventory moves it.
  - **It is immutable with respect to the lot's life.** Editing, cancelling or receiving a
    lot leaves it untouched, which is why a cancelled lot still carries what had been sold
    before it was cancelled, and why a received lot keeps what was sold against it.
  - A SKU's reservations never exceed its quantity **while the lot is scheduled**; a lot
    received short of its reservations carries more reserved than quantity, and both
    listings render that as it is (`002.5` Decision 8, INV-10). The cap was never enforced
    here: **Quantidade** is the lot's and a receipt rewrites it, **Reservas** is physical
    inventory's and does not move, so the two are free to cross.
  - The column carries a `ContextualHelp` in the header whose text — FR-4c's "Unidades já
    vendidas em pedidos e reservadas neste lote." — states what the number is and never
    asserted the cap, so the narrowing changes no copy.
...
- **FR-14a**: The **row overflow menu** is a Shoreline `Menu` anchored to the row's
  ...
  **Marcar como recebido** is offered for **`Agendado` only** (`002.5` FR-1a, Decision 9):
  `Recebido` and `Cancelado` are the lot's past, which is not marked again, and `Inativo` is
  stock the operation parked on purpose — receiving it would push units nobody meant to sell
  into physical inventory, and the way back is to reschedule it in **Editar**. The item is
  **absent**, never disabled. **Editar** and **Cancelar lote** are inert in the prototype —
  neither has a screen in this scope, so picking one only closes the menu and returns focus
  to the trigger. **Marcar como recebido** and **Histórico de alterações** are the two that
  do not stay inert: the first opens the receipt modal (`002.5`), the second is a link out
  of the module, specified in FR-14c.
...
export type FutureInventoryLotItem = {
  ...
  quantity: number
  /**
   * Units already sold in orders. **Read-only**: owned by physical inventory, mirrored
   * here, and never written by any flow in this module (FR-4a, `002.5` FR-18).
   * `<= quantity` **while the lot is scheduled** — a lot received short of its
   * reservations carries more reserved than quantity, and both listings render it
   * (`002.5` Decision 8 / INV-10).
   */
  reserved: number
}
