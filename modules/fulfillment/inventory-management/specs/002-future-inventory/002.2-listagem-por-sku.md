...
- **Acceptance Criteria**:
  - **Given** more than 25 rows, **when** the listing renders, **then** both
    pagination controls report the same window and total — `1 — 25 de 124` — and the
    previous buttons are disabled on the first page. The 231 rows of the fixture become
    124 on screen because the listing opens with `Agendado` + `Inativo` applied
    (`002.1`, FR-7d); the default travels with the view, so switching views never changes
...
- **Acceptance Criteria**:
  - **Superseded by `002.3`, FR-1** — **Given** the page header, **when** I click "Criar lote futuro", **then** nothing
    happens.
  - **Given** a SKU row, **when** I look for an overflow menu, **then** there is none;
    no action applies at SKU granularity.
...
| --- | --- | --- | --- |
| Happy path — browse by SKU | Prototype open, SKU view, no search, no filters beyond the default | Read the first page | 25 flat SKU rows, no expand button, no checkbox; both paginations read `1 — 25 de 124` (`002.1`, FR-7d) |
...
| Happy path — inherited trio repeats | SKU view, no filters | Read the fifteen rows of `#001 Reposição de agosto` | All fifteen show the same destination, `05/08/2026` and `Agendado`; only product and quantity differ |
...
| Happy path — search by lot | SKU view | Type `Reposição de agosto` | The fifteen rows of `#001` remain; pagination reads `15 de 15` |
| Happy path — contextual help | SKU view | Open the help on the "Destino" header | Popover reads "Seller e estoque que receberão o lote." followed by the "Saiba mais" link, opens below the icon and grows leftward, text left-aligned |
| Error case — invalid date range | SKU view, arrival date filter open | Type an end date before the start date, click "Aplicar" | Filter not applied, "A data final não pode ser anterior à data inicial." shown, popover stays open, listing unchanged |
...
| Edge — pagination total changes | Lot view reading `de 38` | Switch to the SKU view | Total becomes `de 124`, 5 pages instead of 2; the last page holds 24 rows |
...
| Edge — inert affordance | SKU view | Click a column header | Sorting does nothing; no console error. "Criar lote futuro" was inert here and now opens the creation form (`002.3`, FR-1) |
| Edge — no row menu | SKU view | Look at the right end of a row | No overflow menu; the 56px column is empty, and the gap to "Status do lote" matches the gap from "Produto" to the left edge |
...
- **FR-4**: A SKU row is the pair **(lot, SKU)** and exposes, in order: Produto
  ...
  (seller over warehouse), Quantidade, Reservas, Chegada and **Status do lote**.
- **FR-4a**: The status column is named **"Status do lote"** in this view, not "Status" as
  in the lot view. On a SKU row, "Status" alone reads as if it could be the SKU's, and a SKU
  has no status of its own — the value is always the lot's (FR-5).
- **FR-4b**: This view has **no row overflow menu**: there is no action that applies at SKU
  granularity, and the lot's actions already live in the view where the lot is the row. The
  trailing **56px column is kept, empty**: it is what makes the gap between the table's
  right edge and "Status do lote" match the gap between its left edge and "Produto".
...
- **FR-6**: The "Destino" column header carries a contextual help immediately after
  ...
  Both follow the pattern of `002.1`, FR-4c — final punctuation, an inline "Saiba mais"
  link to the Help Center, and the popover opening below the icon and growing left — and
  both appear in **this view and the lot view**, since the two columns mean the same thing
  in both.
- **FR-6a**: **Reservas** reports the units of that `(lot, SKU)` pair already sold in
  ...
  belongs to the pair, and the lot view's total is the sum of the pairs.
  - **The number is owned by physical inventory, not by this module** (`002.1` FR-4a,
    `002.5` FR-18). Reservations are made there; both views mirror them so the operator
    does not have to leave future inventory to know what is already committed. No flow
    here changes an existing reservation, and the value is **immutable with respect to
    the lot's life** — editing, cancelling or receiving a lot does not change it.
  - It never exceeds the row's quantity **while the lot is scheduled** — a pair received
    short of its reservations shows more reserved than quantity, and this view renders it
    as it is (`002.5` Decision 8, INV-10). Quantity is the lot's and a receipt rewrites
    it; the reservation is not, and does not move.
- **FR-7**: Quantidade and Reservas are **left-aligned**, as in the lot view
  (`002.1`, FR-4b).
...
- **FR-9a**: **Destino** carries the same **shape** as in the lot view, including the
  single-level variant for accounts with no other sellers connected (`002.1`, FR-7c). The
  shape is decided once per account, so switching views never switches shapes. In such an
  account the **Destino column** of this view also prints the estoque alone, so the column
  and the filter above it never give two different answers about the same account.
...
- **FR-14**: The two empty states of 002.1 apply in this view and both follow Shoreline's
  ...
  "Crie um lote para programar as chegadas" and the primary "Criar lote futuro" — inert
  when this spec was written, routing to the creation form since `002.3`, FR-1 —
...
- **FR-17**: Column sorting and the creation button are rendered but perform no action, and
  bulk selection does not exist. The page header overflow menu is the exception: its
  "Exportar inventário futuro" item is live and opens the export Drawer (`002.4`). There is
  no row menu in this view (FR-4b).
...
- **Amendments**: three, after the requester read it on screen. The copy takes **final
  punctuation** and continues into an inline **"Saiba mais"** link to the Help Center;
  the popover **opens below the icon and grows leftward** instead of rightward, because on
  a right-hand column the rightward panel was clipped by the window; and the help is no
  longer exclusive to this view — **Destino carries it in the lot view too**, since the
  column means the same thing there. The full pattern is in `002.1`, FR-4c.
...
- **Decision**: Keep the 64px row and render the thumbnail at **40px**, the largest
  ...
  `--sl-radius-2`. **The drawing inside is not `image-square`**: `productKind()` reads the
  product name and picks one of a dozen category icons — phone, tv, laptop, audio,
  appliance, accessory and so on — so a page of rows does not repeat the same grey square
  25 times, which would make the column pure decoration. Accessory is tested first, because
  an accessory's name usually cites the device it serves and "Capa para Celular" would
  otherwise become a phone. **This is a fixture artifice, not a product rule**: in
  production the thumbnail is the SKU photo from the catalogue, and `image-square` returns
  as the placeholder for a SKU with no image.
...
- **What the two fields owe the range**: because they are independent pickers, the range
  invariant is not enforced by the widget — it has to be reimplemented on both, and in both
  directions. That is `002.1`, FR-8b: each field bounds the other, an empty field bounds
  nothing, and an empty calendar opens on its bound's month so it never shows a fully
  disabled grid. A `DateRangePicker` would have carried all of this by construction, over a
  single calendar where the second click cannot land before the first. The tradeoff the
  requester chose is still the right one for this popover — two labelled ends, each
  clearable on its own — but it is worth recording that the cost of the choice is exactly
  this rule, and that the rule is the reason the ends of the range have to be edited in a
  particular order once both are set.
...
- **Consequences**: The trigger grows with the length of the first label, and the
  ...
  good share of the filter row. That is what the real Admin would show. Two things
  absorb it: the filter row wraps onto a second line rather than clipping
  (`002.1`, FR-7b), and where the width is fixed — inside the export Drawer — the value
  truncates with an ellipsis and a `Tooltip` carries the full text
  (`002.4`, FR-24). The same Tooltip is wired for the listing's triggers, so a value
  that does get clipped is still readable.

#### Decision 20: The destination filter is a seller → estoque tree

- **Status**: Accepted
- **Context**: The catalogue holds 56 estoques across 48 sellers, and every row read as a
  flat `Seller — Estoque` string. The seller name repeated on every line, and there was no
  way to say "everything that lands on this seller" without checking its estoques one by
  one. The requester asked for a two-level tree: the seller collapsing over its estoques,
  with the chevron between the checkbox and the seller name.
- **Decision**: The list becomes a tree of sellers, each collapsing over its estoques, as
  specified in `002.1`, FR-7a. Paging keeps the shape of Decision 18 with the unit changed:
  a page is **ten sellers**, and a seller's estoques arrive with it rather than counting
  against the page. The chevron is rendered on every seller, even the 40 that hold a single
  estoque — the requester chose the uniform row over the shorter click path. Selection
  stays a flat set of `Seller — Estoque` keys, so nothing downstream of the popover
  changed, Decision 19's summary included.
- **Consequences**: A seller row now has three states instead of two, so its check cell
  renders a dash for the partial case. Because both the seller's state and its toggle read
  **all** of its estoques while the list shows only what the search matched, checking a
  seller can reach rows that are not on screen — that is the honest reading of "this whole
  seller", and it is why the tri-state is never computed over the visible subset.
  Expansion is applied **in place**: opening a seller inserts its estoque rows next to it
  instead of re-rendering the list, for the same reason a new page is appended — a fresh
  `innerHTML` resets `scrollTop`. Two consequences came out of the first build and are now
  part of the rule (`002.1`, FR-7a): the toggle answers to the **whole row minus the
  checkbox**, not to the chevron alone, and the chevron turns by CSS off `aria-expanded`
  instead of the JS swapping one icon for the other — swapping detached the very node the
  click had landed on, and the outside-click handler that runs later on `document` then read
  an orphan target, concluded the click had been outside the filter and closed the popover.
  For the same reason, every outside-click decision now reads the event's `composedPath()`,
  which is snapshotted when the dispatch begins and survives a node being removed mid-click.
  The same tree serves the export Drawer's destination filter (`002.4`, FR-5).
- **Scope of the tree**: two levels are what a **multi-seller** account needs, and the
  requester has since scoped it as such. In an account that holds only its own warehouses,
  the destination filter of both views renders a **single level** — estoques directly, no
  seller row — under `002.1`, FR-7c. The tree stays the default and the collapse depends on
  a signal engineering still owes (`002.1`, Decision 7), so nothing in this decision is
  revoked: it describes the shape this module renders whenever the account has, or might
  have, more than one seller.

#### Decision 21: Three Shoreline contracts that fail silently, and the rule for each

- **Status**: Accepted
- **Context**: Three bugs found on 2026-09-03 turned out to be the same mistake — writing
  what a token or attribute *looks like* it should be instead of what Shoreline actually
  declares. None of them raised an error; each simply produced nothing, which is why they
  survived several rounds of review.
- **Decision**: The three contracts, and the rule that comes out of each.
  1. **`--sl-border-*` are border shorthands, not colours.** `--sl-border-base` resolves to
     `1px solid var(--sl-color-gray-3)`, so `border: 1px solid var(--sl-border-base)`
     expands to `1px solid 1px solid …` — **invalid CSS, dropped by the browser, no border
     at all**. Correct usage is `border: var(--sl-border-base)`. It had bitten the
     `.product-thumb` and `.sku-thumb` frames (Decision 13) and the export card, all of
     which were rendering borderless. When a width other than 1px is genuinely needed, the
     shorthand cannot serve and the underlying palette step has to be named explicitly,
     with a comment saying why.
  2. **Button and IconButton have only `normal` and `large`.** There is no `small`: the
     theme carries no rule for it, so `data-size="small"` yields a button with no element
     height, no padding, no hover and no focus ring — an icon that merely looks like a
     control (`002.4`, FR-13b). `small` *is* valid on `Drawer` and `Modal`, which is what
     made the value look plausible. **A component's size vocabulary is per component.**
  3. **Prefer the semantic token over the palette step, and never write a hex fallback.**
     The export card's status icons used `var(--sl-color-green-8, #1a7f52)` where
     `--sl-fg-success` exists and says what the colour *means*. The hex fallbacks were dead
     code — the tokens exist — and one of them was not even the theme's red, so any day the
     token was renamed the icon would have quietly rendered the wrong colour. For the same
     reason the card's hand-written `box-shadow` became `--sl-shadow-1`, which it had been
     approximating with a different alpha.
- **Consequences**: the general rule is to **read the token's declaration before using it**
  — `--sl-*` names cover colours, shorthands, fonts and shadows, and they are not
  interchangeable. A grep for `solid var(--sl-` and for `data-size="small"` on buttons is
  enough to catch the whole family, and both are clean as of this revision.
...
type FutureInventorySkuRow = {
  ...
  quantity: number
  /**
   * Units of this (lot, SKU) pair already sold in orders. **Read-only**: owned by
   * physical inventory and mirrored here, never written by this module (FR-6a).
   */
  reserved: number
...
no component, and only with `--sl-*` tokens: `.product-thumb` (40px, `--sl-radius-2` and
`--sl-border-base` as the **whole** border shorthand — Decision 13 for the size, Decision 21
for why the shorthand matters) and the positioning rule for the "Destino" contextual help
popover, mirroring `.filter-popover`.
...
| 3 | Destino + contextual help | 170px | 166px — same content as the lot view | `lot.destination.seller` over `lot.destination.warehouse` in `.cell-stack` — the warehouse alone, on one line, in accounts without other sellers (FR-9a); the **header** carries the contextual help of Decision 10 |
| 4 | Quantidade | 101px | 94px — driven by the header, not the number | `row.quantity`, left-aligned (FR-7) |
| 5 | Reservas + contextual help | — | 106px — driven by the header: 62px of text, 4px gap, 24px trigger, plus the 16px separation | `row.reserved`, left-aligned; the **header** carries the contextual help of FR-6 |
| 6 | Chegada | 105px | 97px | `formatDate(lot.arrivalDate)` → `DD/MM/YYYY` |
| 7 | Status do lote | 90px | 106px | `data-sl-tag` from `statusById(lot.status)`; header renamed per FR-4a |
| 8 | *(no label)* | 60px | 56px | **empty** — no row menu in this view (FR-4b); the column survives as the right-hand gutter |
