...
- **Given** the drawer, **when** I press "Adicionar outro produto", **then** another empty row
...
- **FR-1**: The listing's "Criar lote futuro" button — `#btn-create`, inert until now —
  ...
  - **The same route is also reached prefilled**, when a receipt sends its remainder to a new
    lot (`002.5` FR-13): the form opens carrying the SKUs that came short, their missing
    quantities and the origin lot's destination, with the date empty and the form already
    dirty. It is the same screen and the same rules — what differs is that the fields arrive
    filled and the exit modal of FR-28 has something else to warn about.
...
- **FR-4**: Leaving the form — back arrow, "Cancelar", browser back, or a successful save
  — returns to the listing **with its state intact**: search term, filters and view are the
  ones the operator left behind.
  - **A successful save is the one exception, and only for the page: it returns to page 1.**
    The listing is ordered by arrival date, so the new lot lands wherever its date puts it;
    an operator who was on page 3 would come back to page 3 and see no trace of the lot they
    just created, which reads as a save that did not happen. Page 1 does not guarantee the
    lot is on screen either, but it is where the eye goes to look for it. Search, filters
    and view survive the save untouched — and the lot is visible under the default `Status`
    filter by construction, since the form only offers `Agendado` and `Inativo`
    (`002.1`, FR-7d).
...
- **FR-10**: With at least one product, a table appears under the button with three
  ...
  action to remove — an `IconButton` in **`criticalTertiary`**, the variant for a
  destructive action that is not the point of the row. Both columns are **left-aligned**:
  the Quantidade cell holds an `Input`, so the column reads as a field to fill in and not
  as numbers to compare down the page. The table keeps Shoreline's default density — 12px of cell padding,
...
- **FR-11**: The **Quantidade** cell is an `Input` restricted to digits, minimum 1. A cell
  emptied or zeroed is invalid and blocks saving, with **"Informe uma quantidade de 1 ou
  mais"** on the cell. In the drawer the two cases are told apart: an empty field reads
  **"Informe uma quantidade"** and a zero reads **"A quantidade precisa ser 1 ou mais"**.
...
- **FR-13**: Below the table, one line reads **"Total de unidades no lote: N"** — the sum
  of the quantities, updated as they change and never typed. It sits **12px** under the
  last row, in `body` (not `emphasis`) and `fg-base-soft`, with its left edge on the
  **"Produto"** header text, and it repeats the card's top padding below itself so the
  block closes as symmetrically as it opens.
...
- **FR-16**: The drawer content is a list of rows. Each row is a **SKU `Combobox`** and a
  **quantity `Input`**, plus a remove control from the second row onward. It opens with one
  empty row.
- **FR-17**: "Adicionar outro produto" appends an empty row. There is no limit.
...
- **FR-20**: "Adicionar" appends every valid row to the form's table and closes the drawer.
  A row is **empty** (neither field filled), **partial** (one of the two) or **complete**,
  and the three are treated differently:
  - **Empty rows at the end of the list are discarded in silence.** The drawer opens with
    one empty row and "Adicionar outro produto" creates more, so the last unused row is the
    operator answering a button the interface offered — charging them for it would be
    charging them for our own click.
  - **An empty row in the middle blocks the submit**, marked like any other. Between two
    filled rows it is not leftover, it is a gap someone opened and did not close; dropping
    it silently would resolve by guessing.
  - **A partial row always blocks**, wherever it sits: one field filled says the work was
    started, and the missing half is the operator's to supply, not ours to invent.
  - **"Adicionar" stays disabled until at least one row is complete.** With nothing filled
    there is no error to point at — there is nothing to add — so the button says so by
    being unavailable. It is `aria-disabled`, not `disabled`, so a click still explains
    itself: it marks the first row with "Escolha um produto" instead of closing the drawer
    on an empty result.
- **FR-21**: "Cancelar", the dismiss button and `Esc` discard the
  drawer's rows without touching the form. When rows have been filled in, that discard is
  confirmed first. A **click outside the panel does not close the drawer**: dismissal is
  always an explicit action, so a stray click never costs the rows already filled in.
...
- **FR-22a**: In an account whose stock lives **only in its own warehouses**, with no other
  sellers connected, the section shows **one field**: **Estoque**, 350px, and the description
  becomes "Defina em qual estoque esses itens vão estar disponíveis". Seller is not rendered.
  - The dependency of FR-24 has nothing to depend on, so it does not apply: Estoque opens
    enabled, listing every warehouse of the account.
  - Validation asks only for the estoque. The Seller error of FR-25 cannot occur, and the
    focus order of the section is a single field.
  - The saved lot still carries a **full `(seller, warehouse)` pair**, with the account's own
    seller filled in on its behalf (`002.1`, INV-14). The field disappears from the form, not
    from the data — nothing downstream learns that this account renders one level.
  - This is the shape variant of `002.1`, FR-7c, and it is **the same decision, taken once
    per account** for the form and the three filter surfaces. Two levels remain the default:
    the form collapses to one field only on a positive confirmation of the account's
    architecture, which is **an open dependency on engineering** (`002.1`, Decision 7). This
    section is where the two-level default costs the most — a required `Combobox` with a
    single option, blocking the field that follows it — which is why it is worth resolving,
    and equally why guessing is not an option: a lot filed against the wrong seller is a
    delivery promise made against stock that is somewhere else.
...
- **FR-27**: After saving, the operator lands on the listing with a success `Toast`
  naming the lot.
- **FR-28a**: **The browser's back button is a way out of the form like any other, and asks
  the same question.** The `hashchange` handler intercepts a departure from the creation
  route while the form is dirty: it **undoes the navigation first** — putting the route back
  — and only then opens the confirmation, so cancelling returns the operator to the form
  exactly as it was, with every field intact. Confirming marks the form clean, which is what
  lets the second pass through the handler leave without asking again; no control flag is
  needed. A pristine form leaves in silence, as with the back arrow.
  - **An open products drawer is answered first.** Backing out with the drawer open closes
    the drawer — asking about its filled rows, per FR-21 — and keeps the operator on the
    form. One question at a time, and the panel never survives into the listing behind it.
- **FR-28**: "Cancelar" and the back arrow ask for confirmation **only when the form is
  dirty**, through a `ConfirmationModal`. An untouched form leaves silently. The modal is
  titled **"Sair da edição"** and reads **"Todas as informações preenchidas serão perdidas,
  tem certeza que deseja sair?"**; its destructive button is **"Sair"**. The title names the
  action rather than asking a question, since the question is already in the body.
  - **When the form was opened by a receipt, the modal says what leaving actually costs**
    (`002.5` FR-14): the receipt is already committed, so the title, body and destructive
    label become **"Descartar as unidades que não chegaram?"**, **"O lote {code} já foi
    marcado como recebido. Se você sair agora, as {R} unidades que não chegaram serão
    descartadas."** and **"Descartar e sair"**. Losing typed fields and discarding units the
    merchant paid for are not the same warning.
...
| S-2 | Happy path | Drawer open with one empty row | Fill the row; "Adicionar outro produto" twice; fill both; "Adicionar" | Three products join the table in the order they were typed; the drawer closes; the section total is their sum |
...
| S-13 | Edge | Account confirmed to hold only its own warehouses | Open the form and look at "Destino" | One field, **Estoque**, already enabled and listing every warehouse; no Seller field; the description names only the estoque. Saving stores the full pair, with the account's own seller supplied |
| S-14 | Edge | Architecture unknown — the signal of `002.1`, Decision 7 is unavailable | Open the form | Both fields render, Estoque disabled until a Seller is picked: the form never drops the Seller on a missing answer |
...
- **Decision**: `DrawerProvider > DrawerPopover[size="medium"]` with header, content and
  ...
  with "Adicionar outro produto" below and a remove control from the second row on. The footer is
...
6. **Drawer** — the repeater, its rows, "Adicionar outro produto", the footer actions, the
...
| `renderDrawer()` | **new** | Header, rows, "Adicionar outro produto", footer |
...
- **INV-2**: A SKU appears at most once in a lot — enforced in the drawer by Decision 6,
  ...
  uncommitted rows.
- **INV-3**: Every item quantity is an integer of 1 or more at save time. With INV-1 this is
  what keeps a lot from ever holding zero units (`002.1`, INV-12): an emptied or zeroed cell
  blocks the save (FR-11), and so does a zeroed row in the drawer (FR-20).
...
- **INV-7**: `destination.seller` and `destination.warehouse` are always a pair that
  ...
  does not have. This holds when the form renders a single field (FR-22a): the seller is
  supplied from the account rather than chosen, and the pair is still validated against the
  catalogue before the lot is created.
