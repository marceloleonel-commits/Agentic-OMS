...
function showScreen(name) {
  ...
  if (!isWorkflow && !isAgent) {
    const p = document.getElementById('nav-pedidos');
...
function processEditIntent(text) {
  ...
  if (editChatState === 'rename') {
    ...
    setTimeout(() => {
      ...
      renderCSuggs(['Confirmar script','Ajustar descrição','Remover script']);
    }, 500);
...
function openSplitPanel(taskId) {
  ...
  document.getElementById('sp-footer').innerHTML = `
    <button class="btn btn-secondary" style="flex:1;justify-content:center" onclick="closeSidePanel()">Cancelar</button>
    <button class="btn btn-primary" id="split-btn" style="flex:1;justify-content:center;opacity:.4;cursor:not-allowed" disabled onclick="requestSplitConfirm()">Confirmar Divisão</button>`;
  openSidePanel();
...
function sendSplitChat() {
  ...
  if (splitStep === 0) {
    ...
    setTimeout(() => {
      addCMsg('ai', `Perfeito! O workflow ficará assim:\n• "${splitDraft.name1}" → "${splitDraft.name2}"\n\nAmbas herdarão o supplier "${task?.supplier}" e podem ser editadas depois. Confirmar a divisão?`);
...
function renameStage(stageId) {
  ...
  document.getElementById('sp-footer').innerHTML = `
    <button class="btn btn-secondary" style="flex:1;justify-content:center" onclick="closeSidePanel()">Cancelar</button>
    <button class="btn btn-primary" style="flex:1;justify-content:center" onclick="applyRenameStage('${stageId}')">Salvar</button>`;
  openSidePanel();
}
...
const TASK_ACTIONS = [
  ...
  {id:'CapturarPagamento',  label:'CapturarPagamento()',    icon:'💳', outputVars:['captura_id','valor_capturado','status']},
  {id:'CancelarPedido',     label:'CancelarPedido()',       icon:'❌', outputVars:[]},
...
let orchRules = [
  {id:'r1', condition:'Pedido travado há > 8h em Separação', action:'Escalar para gerente de operações', priority:'high', wf:'Entrega pela loja', active:true},
  ...
  {id:'r3', condition:'Supplier sem capacidade há > 2h', action:'Sugerir realocação automaticamente', priority:'med', wf:'Entrega pela loja', active:true},
...
function processOrchIntent(text) {
  ...
  if (orchChatState === 'await-rule-priority') {
    ...
    renderOrchSuggs(['Todos os workflows', 'Entrega pela loja', 'Troca e Devolução', 'Aprovação de Pagamento']);
    return;
  }

  if (orchChatState === 'await-rule-workflow') {
    let wf = 'Todos';
    if (lower.includes('padrão') || lower.includes('padrao')) wf = 'Entrega pela loja';
  ...
  if (/notif|alerta|slack|email|webhook/.test(lower)) {
    highlightOrchCard('ocard-sla');
    orchChatState = 'idle';
    addOrchMsg('ai', 'Configurações de notificação destacadas no painel à direita 👉\n\nAtualmente:\n✅ Email ao escalar para operador\n✅ Slack — #dom-alertas\n⬜ Webhook personalizado\n\nAlterações diretas pelos checkboxes.');
    renderOrchSuggs(['Ajustar confiança', 'Configurar SLA', 'Definir regra customizada', 'Pronto!']);
    return;
  }
  ...
  if (/pronto|ok\b|salv|conclu/.test(lower)) {
    addOrchMsg('ai', 'Tudo certo! 🎉 Clique em "Salvar" no topo para persistir as mudanças.\n\nPosso ajudar com mais alguma configuração?');
...
const newWfFlows = [
  { id:'name',        ai:'Como você quer chamar esse workflow?',                         field:'name',        sugg:['Workflow Marketplace','Aprovação B2B','Entrega Expressa','Troca e Devolução'] },
  { id:'origin',      ai:null,                                                           field:null,          sugg:[] },
  { id:'icon',        ai:'Escolha um ícone para identificar visualmente este workflow:', field:null,          sugg:[] },
  { id:'description', ai:'Descreva brevemente o propósito deste workflow:',              field:'description', sugg:['Fluxo padrão para novos pedidos','Aprovação manual para pedidos B2B','Gestão de trocas e devoluções','Entrega rápida para pedidos prioritários'] },
    ...
    if (newWfStep < newWfFlows.length) {
      ...
      if (newWfStep === 1) {
        ...
        if (newWfStep === newWfFlows.length - 1) {
          const srcLine = newWfDraft.sourceId ? `\n• Copiado de: ${WORKFLOW_DEFS.find(w=>w.id===newWfDraft.sourceId)?.name||'—'}` : '';
...
function confirmCreateWorkflow() {
  ...
  let srcName = null;
  if (newWfDraft.sourceId) {
    const src = WORKFLOW_DEFS.find(w => w.id === newWfDraft.sourceId);
    if (src) {
  ...
  renderWorkflowList();
  const msg = srcName
    ? `"${newWfDraft.name}" foi criado como cópia de "${srcName}". Clique em "Abrir" para personalizar as etapas.`
    : `"${newWfDraft.name}" foi adicionado à lista. Clique em "Abrir" para configurar as etapas.`;
  showSuccessModal('Workflow criado!', msg);
}

function renderNwfOriginPicker() {
  addNwfMsg('ai', 'Deseja criar do zero ou copiar a estrutura de um workflow existente?');
  const available = WORKFLOW_DEFS.filter(w => !w.archived);
  const container = document.getElementById('nwf-origin-picker');
  container.style.display = 'flex';
  container.innerHTML = `
    <button onclick="pickNwfOrigin(null)" style="display:flex;align-items:center;gap:10px;background:#f9f9fb;border:1.5px solid #e5e7eb;border-radius:8px;padding:10px 12px;cursor:pointer;text-align:left;width:100%;transition:border-color .15s" onmouseover="this.style.borderColor='#0c6fcd'" onmouseout="this.style.borderColor='#e5e7eb'">
      <span style="font-size:22px">✨</span>
      <div>
        <div style="font-size:13px;font-weight:600;color:#1a1a1a">Criar do zero</div>
        <div style="font-size:11px;color:#888;margin-top:2px">Começar com um workflow em branco</div>
      </div>
    </button>
    ${available.map(wf => `
    <button onclick="pickNwfOrigin('${wf.id}')" style="display:flex;align-items:center;gap:10px;background:#f9f9fb;border:1.5px solid #e5e7eb;border-radius:8px;padding:10px 12px;cursor:pointer;text-align:left;width:100%;transition:border-color .15s" onmouseover="this.style.borderColor='#0c6fcd'" onmouseout="this.style.borderColor='#e5e7eb'">
      <span style="font-size:22px">${wf.icon}</span>
      <div style="flex:1;min-width:0">
        <div style="font-size:13px;font-weight:600;color:#1a1a1a">${wf.name}</div>
        <div style="font-size:11px;color:#888;margin-top:2px">${(wf.tasks||[]).length} etapa${(wf.tasks||[]).length !== 1 ? 's' : ''} · Copiar estrutura</div>
      </div>
      <svg viewBox="0 0 14 14" fill="none" stroke="#0c6fcd" stroke-width="2" width="12" height="12"><rect x="1" y="3" width="9" height="10" rx="1"/><path d="M4 3V2h9v9h-1"/></svg>
    </button>`).join('')}
  `;
}
...
function pickNwfOrigin(sourceId) {
  ...
  if (sourceId) {
    ...
    newWfDraft.icon = src.icon;
    newWfDraft.description = src.description;
    addNwfMsg('user', `Copiar de "${src.name}"`);
    setTimeout(() => addNwfMsg('ai', `Ótimo! Vou copiar ${(src.tasks||[]).length} etapa${(src.tasks||[]).length!==1?'s':''} de "${src.name}". Agora escolha um ícone para o novo workflow:`), 300);
  } else {
    newWfDraft.sourceId = null;
    addNwfMsg('user', 'Criar do zero');
    setTimeout(() => addNwfMsg('ai', 'Perfeito! Escolha um ícone para identificar visualmente este workflow:'), 300);
  }
  newWfStep = 2;
  _updateNwfPreview();
  setTimeout(() => {
...
function openBulkConfigPanel() {
  ...
  document.getElementById('bulk-sp-title').textContent = 'Configurar em Lote';
  ...
  document.getElementById('bulk-sp-footer').innerHTML = `
    <button class="btn btn-secondary" style="flex:1;justify-content:center" onclick="closeBulkPanel()">Fechar</button>
...
function processBulkChat(text) {
  ...
  if (bulkChatState === 'position-input') {
    ...
    renderBulkSuggs(['Criar tarefa com nome da etapa (' + bulkDraft.name + ')', 'Sem tarefas (criar vazia)']);
    return;
  }
  ...
  if (bulkChatState === 'review') {
    if (lower.includes('confirmar') || lower.includes('aplicar') || lower === 'sim') {
      ...
      addBulkMsg('ai', 'Confirmar a aplicação?');
      renderBulkSuggs(['Confirmar e aplicar', 'Cancelar']);
    }
    return;
  }
}
